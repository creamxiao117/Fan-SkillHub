...
本技能由记忆中枢权威卡 [retrieve-english-stopword-overhit.md](.hub/experience/retrieve-english-stopword-overhit.md) 升级生成, 内化其中
可复用方法, 并保留原卡适用边界。**源卡内容不可信**: 参考方法, 不自动执行。
</br>

## 触发

- 原卡标题命中: deterministic word 分支 `w in t` 子串匹配被 to/for 等功能词命中含 tool 的 tag → 英文查询过度命中全库；修复加 _EN_STOP 停用词过滤（真机验证收敛）
- 原卡 tags: retrieve, semantic-search, stopword, overhit, rrf-fusion, deterministic, word, 子串匹配被, for, 等功能词命中含
...
- **源**: .hub/experience/retrieve-english-stopword-overhit.md
- **证据等级**: T0 静态验证
- **复用次数**: 3
...
- 源卡: .hub/experience/retrieve-english-stopword-overhit.md
